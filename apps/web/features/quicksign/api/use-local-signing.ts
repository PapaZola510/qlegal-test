...
export function useLocalSigning(args: {
	...
	const stampSignature = React.useCallback(async () => {
		const projectId = args.projectId
		if (!projectId) {
			toast.error("No active QuickSign project")
			return
		}
		if (!signaturePngBase64) {
			toast.error("Please provide a signature before stamping")
			return
		}

		setIsStamping(true)
		try {
			await (orpcClient as any).quicksign.stampSignature({
				id: projectId,
				signerEmail: args.signerEmail,
				signaturePngBase64,
			})
			toast.success("Signature stamped on the document")
			args.onSuccess?.()
		} catch (e) {
			toast.error(getOrpcMutationErrorMessage(e, "Could not stamp the signature"))
		} finally {
			setIsStamping(false)
		}
