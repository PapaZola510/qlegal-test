...
import { enpNavItems, isNavItemActive, type NavItem } from "@/features/navigation/nav-config"
...
interface EnpSidebarShellProps {
...
const enpNotifications: Notification[] = [
	{
		id: "e1",
...
const enpQuickActions = [
	...
	{
		label: "Start QuickSign Session",
		href: "/quicksign" as Route,
		icon: SignatureFreeIcons,
	},
	{
		label: "Appointments",
...
const navIconByHref: Record<string, typeof Home01Icon> = {
	...
	"/registry": Shield01FreeIcons,
	"/quicksign": SignatureFreeIcons,
	"/messages": Message01FreeIcons,
}

function EnpSectionLabel({ children }: { children: React.ReactNode }) {
	return (
...
function EnpSidebarNav({ unreadMsgCount }: { unreadMsgCount: number }) {
	...
	const primaryItems = enpNavItems.filter(item => !item.overflow)
	const overflowItems = enpNavItems.filter(item => item.overflow)

	return (
		<>
			<SidebarGroup className="py-1">
				<EnpSectionLabel>Navigation</EnpSectionLabel>
				<SidebarGroupContent>
					<SidebarMenu>
			...
			{overflowItems.length > 0 ? (
				<SidebarGroup className="py-1">
					<EnpSectionLabel>More</EnpSectionLabel>
					<SidebarGroupContent>
						<SidebarMenu>
			...
			<SidebarGroup className="py-1">
				<EnpSectionLabel>Quick Actions</EnpSectionLabel>
				<SidebarGroupContent>
					<SidebarMenu>
						{enpQuickActions.map(action => (
		...
		<Popover>
			...
			<PopoverContent side="bottom" align="end" className="w-80 p-0">
				...
				<div className="max-h-72 space-y-1 overflow-y-auto p-2">
					{enpNotifications.map(notification => (
						<div
							key={notification.id}
...
function EnpUserMenu({ user }: { user: { name: string; email: string; image?: string | null } }) {
...
export function EnpSidebarShell({ user, children }: EnpSidebarShellProps) {
	const pathname = usePathname()
					...
					<SidebarHeader className="border-border flex h-14 shrink-0 flex-row items-center gap-0 border-b p-0 px-3">
						...
						>
							...
							<div className="grid min-w-0 flex-1 text-left leading-tight group-data-[collapsible=icon]:hidden">
								...
								<span className="text-primary truncate text-[11px] font-medium">ENP Portal</span>
							</div>
						</Link>
					</SidebarHeader>

					<SidebarContent>
						<EnpSidebarNav unreadMsgCount={unreadMsgCount} />
					</SidebarContent>

					<SidebarFooter className="border-sidebar-border border-t p-1">
						{user ? <EnpUserMenu user={user} /> : null}
					</SidebarFooter>
					<SidebarRail />
				</Sidebar>
